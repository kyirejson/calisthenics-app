...
export function ProfileScreen({ onPlans, onNutrition }: { onPlans: () => void; onNutrition: () => void }) {
  ...
  const shareBackup = () => void Share.share({ title: '涅槃数据备份', message: exportData() });
  ...
  return <Page><Header eyebrow="仅存储在本机" title="我的" />
    ...
    <Card style={styles.list}><MenuRow icon="▦" title="训练计划" sub={`${plan.shortName} · ${plan.frequency} 练/周`} onPress={onPlans} /><MenuRow icon="◎" title="训练目标" sub={goalName(profile.goal)} /><MenuRow icon="↗" title="我的数据" sub={`${sessions.length} 条训练记录`} /></Card>
    ...
    <Text style={styles.version}>涅槃 App · v1.1.0{`\n`}数据默认不上传云端</Text>
  </Page>;
}
...
function goalName(goal: string) { return ({ strength: '提升力量', gain: '增肌塑形', fat_loss: '减脂塑形', health: '健康体能' } as Record<string,string>)[goal] || goal; }
