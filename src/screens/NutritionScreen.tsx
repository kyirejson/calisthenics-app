...
export function NutritionScreen({ onBack }: { onBack: () => void }) {
  ...
  return <SafeAreaView style={styles.safe}>
    <View style={styles.top}><Pressable onPress={onBack} style={styles.back}><Text style={styles.backText}>‹</Text></Pressable><Text style={styles.title}>营养赋能</Text><View style={{ width: 42 }} /></View>
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      <Text style={styles.hero}>吃得清楚，{`\n`}才能练得长久。</Text>
      <Text style={styles.sub}>训练目标、营养目标和饮食模式分别计算。这里展示的是今日计划，并不代表你已经吃过。</Text>
      ...
      <View style={styles.energyCard}>
        ...
        <View style={styles.energyRow}><Mini label="基础代谢" value={`${plan.bmr}`} /><Mini label="维持消耗" value={`${plan.tdee}`} /><Mini label="饮水" value={`${(plan.waterMl / 1000).toFixed(1)}L`} /></View>
      </View>
...
const styles = StyleSheet.create({
  ...
  content: { padding: 20, paddingBottom: 55 }, hero: { color: colors.ink, fontSize: 34, lineHeight: 41, fontWeight: '900', letterSpacing: -1.1 }, sub: { color: colors.inkMuted, fontSize: 14, lineHeight: 22, marginTop: 10 }, sectionTitle: { color: colors.ink, fontSize: 19, fontWeight: '900', marginTop: 26, marginBottom: 12 }, horizontal: { paddingRight: 20 },
