...
const goals: Array<{ key: Goal; label: string; desc: string }> = [
  ...
  { key: 'cut', label: '减脂', desc: '力量训练搭配能量管理' },
    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          <View style={styles.brand}><Text style={styles.brandMark}>Z</Text><Text style={styles.brandText}>自重进阶</Text></View>
          ...
          {step === 0 ? (
            <View>
              ...
              <View style={styles.inline}><View style={{ flex: 1 }}><Field label="年龄" value={age} onChangeText={setAge} keyboardType="number-pad" /></View><View style={{ width: 12 }} /><View style={{ flex: 1 }}><Field label="身高 cm" value={height} onChangeText={setHeight} keyboardType="decimal-pad" /></View></View>
              <Field label="体重 kg" value={weight} onChangeText={setWeight} keyboardType="decimal-pad" />
              <View style={styles.pills}><Pill label="男性" active={sex === 'male'} onPress={() => setSex('male')} /><Pill label="女性" active={sex === 'female'} onPress={() => setSex('female')} /></View>
            </View>
          ) : step === 1 ? (
            <View>
...
const styles = StyleSheet.create({
  ...
  brandMark: { width: 34, height: 34, borderRadius: 10, backgroundColor: colors.lime, color: colors.ink, textAlign: 'center', lineHeight: 34, fontWeight: '900', fontSize: 20 },
  ...
  input: { backgroundColor: '#262822', color: '#FFFFFF', height: 54, borderRadius: radius.sm, paddingHorizontal: 16, fontSize: 17, borderWidth: 1, borderColor: '#373A32' },
  inline: { flexDirection: 'row' }, pills: { flexDirection: 'row', marginTop: 2 },
