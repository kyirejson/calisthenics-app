...
export function RunScreen({ onBack, onComplete }: { onBack: () => void; onComplete: () => void }) {
  ...
  const plannedDay = profile ? getPlanDay(profile).day : null;
  ...
  const begin = async () => {
    startedAt.current = new Date(); statusRef.current = 'running'; setStatus('running');
    try {
      const result = await Location.requestForegroundPermissionsAsync();
      if (result.status !== 'granted') {
        setPermission('denied');
        showMessage('仅记录运动时间', `未授权定位，仍可完成${weightLoss ? '快走或骑行' : '快走或轻松跑'}计时；距离和配速不会记录。`);
        return;
      }
  ...
  const finish = async () => {
    ...
    const session: TrainingSession = { id: `run_${Date.now()}`, workoutId: 'outdoor_run', workoutName: plannedDay?.type === 'cardio' ? plannedDay.title : '有氧活动', startedAt: (startedAt.current || new Date()).toISOString(), completedAt: new Date().toISOString(), durationSeconds: elapsed, exercises: [], totalReps: 0, kind: 'running', distanceKm: Number(km.toFixed(2)), calories: weightLoss ? undefined : calories, completion: targetMinutes && elapsed < targetMinutes * 60 ? 'partial' : 'complete' };
  ...
  return <LinearGradient colors={['#151712', '#252A1D']} style={styles.gradient}><SafeAreaView style={styles.safe}>
    <View style={styles.top}><Pressable onPress={onBack} style={styles.back}><Text style={styles.backText}>‹</Text></Pressable><Text style={styles.title}>{weightLoss ? '步行与有氧记录' : '有氧记录'}</Text><View style={{ width: 42 }} /></View>
    {targetMinutes ? <Text style={styles.target}>今日目标：可交谈强度活动 {targetMinutes} 分钟 · 已完成 {Math.floor(elapsed / 60)} 分钟</Text> : null}
